...
class MultiScaleDecoder(nn.Module):
    def __init__(self, num_classes, backbone, BatchNorm, scales):
        ...
        self.scales = scales
        self.keys = {scale:str(i) for i, scale in enumerate(self.scales)}
        self.conv_low_level = nn.ModuleDict({self.keys[scale] : nn.Sequential(nn.Conv2d(low_level_inplanes, 48, 1, bias=False),
                                             BatchNorm(48),
                                             nn.ReLU()) 
                                             for scale in self.scales})

        self.conv_last = nn.ModuleDict({self.keys[scale] : nn.Sequential(nn.Conv2d(304, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       BatchNorm(256),
                                       nn.ReLU(),
                                       nn.Dropout(0.5),
                                       nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
    ...
    def forward(self, aspp_output, low_level_feat):
        outputs = {}
        for scale in self.scales:
            low = self.conv_low_level[self.keys[scale]](low_level_feat)
            ...
            y = self.conv_last[self.keys[scale]](x)
            outputs[scale] = y
