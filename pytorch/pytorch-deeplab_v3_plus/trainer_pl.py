    ...
    def get_loss(self, batch, batch_idx):
        ...
        if self.hparams.densecrfloss==0:
            ...
            for scale, probs in scale_probs.items():
                ...
                scale_rloss[scale] = self.hparams.densecrfloss*self.hparams.rloss_weights[scale]*self.CRFLoss[scale](denormalized_image, rescaled_probs, scaled_roi)
            ...
            for scale, probs in probs_copy.items():
                ...
                denormalized_image = denormalizeimage(scaled_img, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225))
                scaled_roi = F.interpolate(croppings.unsqueeze(0), size=scaled_size, mode='nearest').squeeze(0)
                rloss_copy[scale] = self.hparams.densecrfloss*self.hparams.rloss_weights[scale]*self.CRFLoss[scale](denormalized_image, rescaled_probs, scaled_roi)
